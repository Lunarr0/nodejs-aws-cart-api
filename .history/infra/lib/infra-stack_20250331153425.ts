...
export class InfraStack extends cdk.Stack {
  ...
  const nestLambda = new lambda.Function(this, 'NestJsLambda', {
    ...
    handler: 'dist/lambda.handler',  // Updated handler path
      ...
      bundling: {
        ...
        command: [
          'bash', '-c', [
            ...
            'cp package*.json /asset-output/',  // Copy package files
            'cd /asset-output',                 // Move to output directory
            'npm ci --production',              // Install only production dependencies
            'cp -r /asset-input/dist .',        // Copy dist folder to output
