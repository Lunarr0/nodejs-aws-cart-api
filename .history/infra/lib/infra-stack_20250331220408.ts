...
export class InfraStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const nestLambda = new lambda.Function(this, 'NestJsLambda', {
      ...
      handler: 'dist/src/lambda.handler', // This should match your Webpack output (dist/src/lambda.js)
      code: lambda.Code.fromAsset(path.join(__dirname, '../../dist'), { // Ensure it points to the correct output directory
        exclude: [
          '.git',
          'test',
          'cdk.out',
          ...
          local: {
            tryBundle(outputDir: string) {
              try {
                ...
                const commands = [
               // Create fresh directory for node_modules
               `rm -rf ${outputDir}/node_modules`,
              
               // Copy the compiled lambda file to root
               `cp ${projectRoot}/dist/src/lambda.js ${outputDir}/lambda.js`,
               
               // Copy package files
               `cp ${projectRoot}/package.json ${outputDir}/`,
               `cp ${projectRoot}/package-lock.json ${outputDir}/`,
               
               // Copy existing node_modules (faster than reinstalling)
               `cp -r ${projectRoot}/node_modules ${outputDir}/`,
