...
export class InfraStack extends cdk.Stack {
  ...
  const nestLambda = new lambda.Function(this, 'NestJsLambda', {
    ...
    code: lambda.Code.fromAsset(path.join(__dirname, '../../dist'), {
      ...
      exclude: ['*.ts', '*.map'],
      bundling: {
        image: cdk.DockerImage.fromRegistry('dummy'), // Dummy image that won't be used
        command: ['echo "Docker not used"'], // Dummy command that won't be used
        local: {
          tryBundle(outputDir: string) {
            try {
              const commands = [
                `cp -r ${path.join(__dirname, '../../dist')}/* ${outputDir}/`,
