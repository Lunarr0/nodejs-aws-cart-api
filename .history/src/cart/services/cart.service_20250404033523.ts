...
import { PutCartPayload, CreateOrderPayload , OrderStatuses} from 'src/order/type';
...
export class CartService {
  ...
  // new
  async updateCartStatusByUserId(userId: string, status: CartStatuses): Promise<void> {
    try {
      this.logger.debug(`Updating cart status for userId: ${userId} to ${status}`);
      ...
      return this.dataSource.transaction(async (transactionalEntityManager) => {
        try {
          ...
          this.logger.debug(`Cart status updated to ${status} for cart ID: ${cart.id}`);
        } catch (error) {
          this.logger.error('Transaction error while updating cart status:', error);
          throw error;
        }
      });
  
    } catch (error) {
      this.logger.error(`Error updating cart status for user ${userId}:`, error);
      throw error;
    }
  }
  ...
  //           const existingCartItem = await transactionalEntityManager.findOne(CartItem, {
  //             where: {
  //               cart_id: cart.id,
  //               product_id: product.id
  //             }
  ...
  async removeByUserId(userId: string): Promise<void> {
    try {
      this.logger.debug(`Updating cart status for userId: ${userId}`);
      const cart = await this.findByUserId(userId);
      ...
      return this.dataSource.transaction(async (transactionalEntityManager) => {
        try {
          this.logger.debug(`Updating cart status with ID: ${cart.id}`);
          await transactionalEntityManager.update(Cart, cart.id, {
            status: CartStatuses.ORDERED,
            updated_at: new Date()
          });
          this.logger.debug('Cart status updated successfully');
        } catch (error) {
          this.logger.error('Transaction error:', error);
          throw error;
        }
