...
import { Cart, CartItem, Order, OrderStatus } from '../../entities';
    ...
    try {
      ...
      return this.dataSource.transaction(async (transactionalEntityManager) => {
        try {
          ...
          let product = await transactionalEntityManager.findOne('products', {
          ...
          if (!product) {
            ...
            const newProduct = transactionalEntityManager.create('products', {
              title: payload.product.title,
              description: payload.product.description,
              price: payload.product.price
            });
            product = await transactionalEntityManager.save('products', newProduct);
          ...
          await transactionalEntityManager.update(Cart, cart.id, {
            updated_at: new Date() // Changed from Date.now() to new Date()
          });
